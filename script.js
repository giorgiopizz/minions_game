...
class Obj {
	constructor(team, x, y, shape, side, velocity, dirx, diry) {
		this.team = team;
		this.x = x;
		this.y = y;
		this.shape = shape;
	...
	move(t) {
			this.x += this.velocity * this.dirx * t;
			this.y += this.velocity * this.diry * t;
			this.draw();
	}
	draw() {
		ctx.beginPath();
...
class ObjFollower {
	constructor(team, x, y, shape, side, velocity) {
		this.team = team;
		this.x = x;
		this.y = y;
		this.shape = shape;
	...
	move(objects){
			// var objs = objects.filter((obj) => obj.team !== this.team);
			var objsDistances = objects.map((obj) => {
				return {
					obj: obj,
					distance: Math.sqrt(
			...
			if (
				...
				if (nearest.distance <= this.side + otherSide + tolerance) {
					this.draw();
					return;
				}
...
function destroy(objects){
	var toBeRemoved = [];
	objects.forEach(obj=>{
		if (obj.x>canvas.width || obj.x<0 || obj.y>canvas.height || obj.y<0){
...
function main() {
	...
	var objects = [];
	const velocity = 1;
	objects.push(new Obj(0, offsetX, 100, "rect", size, 0, 0, 1));
	objects.push(new Obj(0, offsetX + posX, 100, "rect", size, 0, 1, 1));
	objects.push(new Obj(0, offsetX + 2 * posX, 100, "rect", size, 0, 1, 1));
	objects.push(new Obj(1, offsetX, posY, "rect", size, 0, 0, 1));
	objects.push(new Obj(1, offsetX + posX, posY, "rect", size, 0, 1, 1));
	objects.push(new Obj(1, offsetX + 2 * posX, posY, "rect", size, 0, 1, 1));
	...
	function mouseClickHandler(e) {
		if (team ===0){
			team = 1;
			objects.push(
				new ObjFollower(0, newPosX, newPosY, "ball", size / 5, velocity)
			);
		}
		else{
			team = 0;
			objects.push(
				new ObjFollower(1, newPosX, newPosY, "ball", size / 5, velocity)
			);
		}
	}
